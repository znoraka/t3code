...
| `packages/client-runtime/src/connection/supervisor.ts`                       | Wake probe routed through `_lempire` `wakeProbe`; keepalive `heartbeatMonitor` raced with the lease monitor; retry-ladder override in `retryDelayMs`. | Mobile connection resilience |
| `packages/client-runtime/src/connection/index.ts`                            | Export of `enableMobileConnectionResilience`.                                                                                                 | Mobile connection resilience |
| `apps/mobile/src/connection/platform.ts`                                     | Import + module-load call of `enableMobileConnectionResilience()`.                                                                            | Mobile connection resilience |
...
| Mobile connection resilience | `packages/client-runtime/src/connection/_lempire/connectionResilience.ts` (+ `.test.ts`) | Diagnosed 2026-08-21 with a two-path websocket bench (public vs Tailscale): the constant mobile disconnects were app-made, not network-made. Three tunings, all inert until `enableMobileConnectionResilience()` (mobile platform layer calls it at module load), so upstream supervisor tests run unchanged: **(1)** a keepalive heartbeat probes the live session every 15s and fails the lease after 2 consecutive misses — upstream never pings an established socket, so idle flows on VPN/tailnet paths get reaped silently and the zombie survives until the next user action; **(2)** the `application-active-probe` wake probe gets 3 × 4s attempts (transient failures retried) instead of a single 3s shot — a healthy tailnet path can exceed 3s right after device wake, so the single shot tore down good connections; **(3)** the reconnect ladder starts at 1s instead of 3s. The heartbeat is raced inside the supervisor's connected-lease monitor, deliberately **not** implemented as periodic wakeups: wakeups with reason `application-active-probe` also re-trigger thread/shell resubscription (`shouldResubscribeAfterWakeup`), which would cause a sync burst every interval. |
