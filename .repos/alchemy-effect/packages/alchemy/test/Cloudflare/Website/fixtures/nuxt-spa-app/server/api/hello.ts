// A nitro server route. With `ssr: false` nitro still keeps `server/`
// routes running in the worker — only page rendering moves to the browser.
// `event.context.cloudflare` is the cloudflare_module preset's runtime
// contract: `{ request, env, context }`.
//
