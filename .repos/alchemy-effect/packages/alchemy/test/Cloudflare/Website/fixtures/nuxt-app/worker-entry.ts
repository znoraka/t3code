 ...
 *
 * `NuxtProps.main` maps this module onto `nitro.options.entry`, so nitro's
 * rollup bundles THIS module as the worker entry: its exports are the
 * worker's exports. It wraps nitro's emitted cloudflare-module handler
 ...
 *
 * NOTE: like the rest of this fixture, the file is excluded from the
 * alchemy test project's type-check (tsconfig.test.json) — it only
 * compiles inside a nuxt build, where nitro provides the aliases.
 */
