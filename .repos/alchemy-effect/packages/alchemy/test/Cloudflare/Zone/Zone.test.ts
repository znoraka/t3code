...
// the block is account-level and can persist for hours. The zone-provisioning
// tests below create fresh zones, so soft-skip them while the account is in
...
const softSkipWhenZoneCreationBlocked = <A, E, R>(
  ...
  effect.pipe(
    Effect.catchCause((cause) => {
      ...
      // error can be constructed inside the provider's copy of the distilled
      // module, so `instanceof` across that module boundary can be false.
      ...
      return blocked
        ? Effect.logWarning(
            "Cloudflare has zone creation blocked on this account (error 1052) — skipping zone-provisioning assertions",
...
test.provider.skipIf(!!process.env.FAST)(
  ...
  (stack) =>
    softSkipWhenZoneCreationBlocked(
      Effect.gen(function* () {
