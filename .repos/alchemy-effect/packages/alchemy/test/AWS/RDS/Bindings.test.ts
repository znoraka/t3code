...
import RdsBindingsTestFunctionLive, {
  RdsBindingsTestFunction,
} from "./fixtures/handler";

const testOptions = { providers: AWS.providers() };
const { test, beforeAll, afterAll } = Test.make(testOptions);
const sharedStack = Core.scratchStack(testOptions, "RdsBindings");
...
describe.sequential("RDS Bindings", () => {
  beforeAll(
    Effect.gen(function* () {
      yield* Effect.logInfo("RDS test setup: destroying previous resources");
      yield* sharedStack.destroy();

      yield* Effect.logInfo("RDS test setup: deploying fixture");
      const { functionUrl } = yield* sharedStack.deploy(
        Effect.gen(function* () {
          return yield* RdsBindingsTestFunction;
        }).pipe(Effect.provide(RdsBindingsTestFunctionLive)),
      );
  ...
  describe("DescribeEvents", () => {
    test.provider("lists the account's recent RDS events", (_stack) =>
      Effect.gen(function* () {
  ...
  describe("DescribeDBClusterSnapshots", () => {
    test.provider("lists the account's cluster snapshots", (_stack) =>
      Effect.gen(function* () {
        const response = yield* getJson("/cluster-snapshots");
        expect((response as any).count).toBeGreaterThanOrEqual(0);
      }),
    );
  });
  ...
  describe("DescribeDBSnapshots", () => {
    test.provider("lists the account's instance snapshots", (_stack) =>
      Effect.gen(function* () {
        const response = yield* getJson("/instance-snapshots");
        expect((response as any).count).toBeGreaterThanOrEqual(0);
      }),
    );
  });
  ...
  describe("DeleteDBClusterSnapshot", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = yield* getJson("/delete-cluster-snapshot-probe");
  ...
  describe("CopyDBClusterSnapshot", () => {
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          const response = yield* getJson("/copy-cluster-snapshot-probe");
          expect((response as any).tag).toBe("DBClusterSnapshotNotFoundFault");
        }),
    );
  });

  describe("DeleteDBSnapshot", () => {
    test.provider(
      "surfaces the typed not-found tag for a nonexistent snapshot",
      (_stack) =>
        Effect.gen(function* () {
          const response = yield* getJson("/delete-instance-snapshot-probe");
          expect((response as any).tag).toBe("DBSnapshotNotFoundFault");
        }),
    );
  });

  describe("CopyDBSnapshot", () => {
    test.provider(
      "surfaces the typed not-found tag for a nonexistent source snapshot",
      (_stack) =>
        Effect.gen(function* () {
          const response = yield* getJson("/copy-instance-snapshot-probe");
          expect((response as any).tag).toBe("DBSnapshotNotFoundFault");
        }),
    );
  });
    ...
    test.provider(
      ...
      (_stack) =>
        Effect.gen(function* () {
          ...
          const arn = `arn:aws:rds:${region}:${identity.Account}:cluster:alchemy-nonexistent-rds-cluster-probe`;
