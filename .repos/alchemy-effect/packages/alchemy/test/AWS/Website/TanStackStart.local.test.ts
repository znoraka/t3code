...
describe("AWS.Website.TanStackStart local", () => {
  test.provider(
    "dev runs TanStack Start's own Vite dev server with no cloud resources",
    (stack) =>
      Effect.gen(function* () {
        yield* stack.destroy();

        const rootDir = yield* cloneFixture(fixtureDir, {
          prefix: "alchemy-tanstack-start-aws-local-",
          tempRoot,
  ...
  test.provider(
    ...
    (stack) =>
      Effect.gen(function* () {
        ...
        const rootDir = yield* cloneFixture(fixtureDir, {
          prefix: "alchemy-tanstack-start-aws-local-ssr-",
          tempRoot,
          entries: fixtureEntries,
        });
        ...
        // SSR page served by the TanStack Start dev server (native HMR).
