...
/**
 * `StartTargetedSentimentDetectionJob` request with `DataAccessRoleArn` defaulting to the role the
 * binding was constructed with.
 */
export interface StartTargetedSentimentDetectionJobRequest extends Omit<
  comprehend.StartTargetedSentimentDetectionJobRequest,
  "DataAccessRoleArn"
> {
  /**
   * IAM role Amazon Comprehend assumes to read the input documents and write
   * the results.
   * @default the data-access role bound via `StartTargetedSentimentDetectionJob(role)`
   */
...
export interface StartTargetedSentimentDetectionJob extends Binding.Service<
  ...
  <R extends Role>(
    ...
    (
      ...
      comprehend.StartTargetedSentimentDetectionJobResponse,
      comprehend.StartTargetedSentimentDetectionJobError
    >
  >
> {}
export const StartTargetedSentimentDetectionJob =
  Binding.Service<StartTargetedSentimentDetectionJob>(
    "AWS.Comprehend.StartTargetedSentimentDetectionJob",
  );
