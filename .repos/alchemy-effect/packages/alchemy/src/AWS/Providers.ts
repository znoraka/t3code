...
export const providers = () =>
  Layer.effect(
    ...
    // live chain provided at the bottom of this pipe) — see
    // `pinCollectionEnvironment`. The ambient environment the STACK program
    // sees flips to the emulator in a dev run (bottom of this file), and the
    // pin is what keeps live-mode and mode-agnostic providers on the real
    // cloud regardless.
    Effect.flatMap(
      Provider.collection([
        KeyPair,
      ...
      (collection) =>
        Effect.map(captureAwsEnvironment, (environment) =>
          pinCollectionEnvironment(collection, environment),
        ),
    ),
  ).pipe(
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.mergeAll(
          ...
          flociDual(ApplicationAutoScaling.ScalableTarget, () =>
            ApplicationAutoScaling.ScalableTargetProvider(),
          ),
          flociDual(ApplicationAutoScaling.ScalingPolicy, () =>
            ApplicationAutoScaling.ScalingPolicyProvider(),
          ),
          flociDual(ApplicationAutoScaling.ScheduledAction, () =>
        ...
        Layer.mergeAll(
          ...
          flociDual(ECS.CapacityProvider, () => ECS.CapacityProviderProvider()),
          ...
          flociDual(Glue.Crawler, () => Glue.CrawlerProvider()),
          flociDual(Glue.Database, () => Glue.DatabaseProvider()),
          flociDual(Glue.Job, () => Glue.JobProvider()),
          flociDual(Glue.Table, () => Glue.TableProvider()),
        ...
        Layer.mergeAll(
          ...
          flociDual(Logs.Destination, () => Logs.DestinationProvider()),
          ...
          flociDual(Pipes.Pipe, () => Pipes.PipeProvider()),
          ...
          flociDual(RDS.DBSubnetGroup, () => RDS.DBSubnetGroupProvider()),
          ...
          flociDual(SES.ConfigurationSet, () => SES.ConfigurationSetProvider()),
          flociDual(SES.ContactList, () => SES.ContactListProvider()),
          flociDual(SES.Contact, () => SES.ContactProvider()),
          flociDual(SES.CustomVerificationEmailTemplate, () =>
            SES.CustomVerificationEmailTemplateProvider(),
          ),
    ...
    // In an `alchemy dev` run the AMBIENT environment — what stack
    // composition code, plan-time data sources (`AWS.EC2.getAmi`), and any
    // raw distilled call see — is the EMULATOR, so dev needs no real
    // credentials and no call-site routing. The providers above were built
    // beneath the live chain and keep it (their lifecycle ops close over
    // their build context): live-mode and mode-agnostic resources still hit
    // the real cloud, and local variants override per-resource via their
