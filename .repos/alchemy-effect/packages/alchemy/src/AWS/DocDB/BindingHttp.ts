...
/**
 * Shared scaffolding for Amazon DocumentDB HTTP bindings.
 *
 ...
 *
 * DocumentDB shares the RDS control plane, so its IAM actions live under the
 * `rds:` service prefix (`rds:FailoverDBCluster`, `rds:DescribeDBClusters`,
 * …) and its ARNs use the `rds` service segment.
 */
 ...
 * read-only monitoring actions span every cluster in the account and the
 * identifiers they filter on are runtime data.
 */
export const makeDocDBAccountHttpBinding = <I, A, E, R>(options: {
  /** Fully-qualified binding tag, e.g. `AWS.DocDB.DescribeEvents`. */
  tag: string;
 ...
 * creation).
 */
export const makeDocDBClusterHttpBinding = <
  I extends { DBClusterIdentifier?: string },
  A,
  E,
  R,
>(options: {
  /** Fully-qualified binding tag, e.g. `AWS.DocDB.FailoverDBCluster`. */
  tag: string;
 ...
 * instance ARN.
 */
export const makeDocDBInstanceHttpBinding = <
  I extends { DBInstanceIdentifier?: string },
  A,
  E,
  R,
>(options: {
  /** Fully-qualified binding tag, e.g. `AWS.DocDB.RebootDBInstance`. */
  tag: string;
            ...
            policyStatements: [
              {
                Effect: "Allow",
                Action: [...options.actions],
                Resource: [Output.interpolate`${instance.dbInstanceArn}`],
              },
            ],
          });
        }
