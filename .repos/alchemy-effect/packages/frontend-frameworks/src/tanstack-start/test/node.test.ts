...
describe("makeNodeTarget", () => {
  it("declares the node platform, Node bundle conditions, and a finish pass", () => {
    ...
    expect(node.serverEntryFileName).toBe(SERVER_ENTRY_FILE_NAME);
    expect(SERVER_ENTRY_FILE_NAME).toBe("server.js");
  ...
  it("carries the outDir override on its config", () => {
    expect(makeNodeTarget({ outDir: "build" }).config.outDir).toBe("build");
  });
    ...
    const output = await runWithNode(
      Effect.gen(function* () {
        ...
        const dir = yield* fs.makeTempDirectoryScoped({
          prefix: "tanstack-start-node-finish-",
        });
