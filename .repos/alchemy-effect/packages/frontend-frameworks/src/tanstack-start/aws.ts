/**
 * `@alchemy.run/frontend-frameworks/tanstack-start/aws` — the AWS Lambda
 * deploy target for the TanStack Start integration.
 *
 ...
 *   classifies `.js` by the nearest manifest, and `<outDir>/server` ships
 *   without one), a copy of this package's self-contained `aws-lambda` web
 *   adapter, and `index.mjs` exporting `handler` — the fetch handler wrapped
 *   with `awslambda.streamifyResponse` (or the buffered payload-v2 wrapper
 *   with `streaming: false`). The rewritten `BuildOutput` puts
 *   `server/index.mjs` first, so it becomes the deployed Lambda entry.
 ...
 *   server code.
 *
 * No `devPlatform`: `dev` runs TanStack Start's own Vite dev server, which
...
export const LAMBDA_ADAPTER_FILE_NAME = "tanstack-start-aws-lambda.mjs";

/** AWS-specific knobs carried on the shared {@link TanStackStartTargetConfig}. */
export interface TanStackStartAwsTargetConfig extends TanStackStartTargetConfig {
  /**
...
/**
 * The generated Lambda entry: imports TanStack Start's server entry and
 * exports its fetch handler wrapped for the Lambda Node.js runtime.
 *
 * The entry module default-exports `{ fetch }` (the shape
 * `createServerEntry` produces). A project whose custom server entry
 * default-exports the bare fetch function is accepted too.
 */
export const generateLambdaEntry = (options: {
  ...
  return `// Auto-generated by @alchemy.run/frontend-frameworks/tanstack-start/aws — do not edit.
...
const finish = (
  config: TanStackStartAwsTargetConfig,
  output: BuildOutput,
...
> =>
  Effect.gen(function* () {
    ...
    // without one — the ESM entry and its chunks need the marker.
 ...
 */
export interface TanStackStartAwsBuildChildConfig {
  readonly rootDir: string;
  /** The (JSON-serializable) target config the parent was created with. */
  readonly config: TanStackStartAwsTargetConfig;
}

export const buildInChild = (config: TanStackStartAwsBuildChildConfig) =>
  Effect.gen(function* () {
...
/**
 * Create the AWS Lambda {@link TanStackStartTarget}. See the module doc for
 * the seams.
 */
  ...
  build: (context) =>
    runBuildChild({
      ...
      config: {
        rootDir: context.root,
        config,
      } satisfies TanStackStartAwsBuildChildConfig,
    }).pipe(Effect.mapError((error) => fail(error.message, error.cause))),
});
