...
for (const mode of Playwright.SERVER_METHODS) {
  test.describe(mode, () => {
    ...
    it("SSRs the homepage with the docs sidebar", async ({ page, server }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Vocs Fixture Home" }),
      ).toBeVisible();
      ...
      await expect(
        sidebar.getByRole("link", { name: "Counter" }),
      ).toBeVisible();
    });

    it("navigates to a second page from the sidebar", async ({
      page,
      server,
    }) => {
    ...
    it("hydrates the custom MDX React component", async ({ page, server }) => {
      ...
      await expect(page.getByTestId("counter-page-marker")).toHaveText(
        "COUNTER_PAGE_MARKER",
      );
