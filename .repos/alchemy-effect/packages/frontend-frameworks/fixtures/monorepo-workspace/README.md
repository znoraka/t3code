# packages/frontend-frameworks/fixtures/monorepo-workspace
...
```
packages/frontend-frameworks/fixtures/monorepo-workspace/   ← fixture root (bun workspace member; harness cwd)
...
- **Relative import, no bun workspace-protocol tricks.** `lib/` is NOT a bun
  workspace member (the root glob is `packages/frontend-frameworks/fixtures/*`, one level up). The app
