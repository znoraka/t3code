...
for (const mode of Playwright.SERVER_METHODS) {
  test.describe(mode, () => {
    ...
    it("revalidatePath purges the writable cache (on-demand ISR)", async ({
      server,
    }) => {
      ...
      expect((await revalidate.json()) as object).toEqual({
        revalidated: true,
      });
    ...
    it("time-based revalidation regenerates through the DO queue", async ({
      server,
    }) => {
      test.setTimeout(90_000);
      // Prime the 2s-revalidate page.
      const primed = await pollStamp(
        server,
        "/fast-isr",
        "fast-isr-stamp",
        () => true,
