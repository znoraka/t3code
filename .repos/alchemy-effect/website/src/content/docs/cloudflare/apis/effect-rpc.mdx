...
it inside the constructor — `bind()` returns a typed handle whose `get` /
...
> request. The constructor only constructs; the work happens later, on each
> `fetch` call.
...
The DO's constructor returns `{ fetch }` produced by
