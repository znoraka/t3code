...
You build that stack once in the Runtime's **Construction phase** and
...
the Construction/Runtime split, `.env`, and transformations.

Because the value is in scope right in the constructor, the model Layer can be
...
Cloudflare.Workers.AI()` in the Construction phase attaches the binding, and
...
Bind the gateway in the Construction phase and call `aiGateway.model({...})`
...
bottom of the constructor's layer chain so the binding resolves at runtime:
