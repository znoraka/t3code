...
export const drizzleDeps = `drizzle-orm@1.0.0-rc.5-ab785fc @effect/sql-pg pg`;
export const drizzleDevDeps = `drizzle-kit@1.0.0-rc.5-ab785fc @types/pg`;
...
import time against it. Don't widen the version range until drizzle
...
The MySQL flavor changes three things: the schema dialect, the
migration dialect, and the runtime driver. Start with the schema —
same tables, `mysql-core` column builders:
...
Swap the runtime driver dependency — `@effect/sql-mysql2` and
`mysql2` in place of `@effect/sql-pg` and `pg`:
...
`src/Db.ts` keeps `Drizzle.Schema` (with `dialect: "mysql"`) and
uses the MySQL resource trio — database, branch, and a
...
At runtime, `Drizzle.MySQL` is the drop-in sibling of
`Drizzle.Postgres` — the same connection-string wiring, backed by
`@effect/sql-mysql2`:
...
On Workers the client defaults to the text protocol (Hyperdrive's
MySQL proxy has no `COM_STMT_PREPARE`) and eval-free row parsing
(the isolate forbids `new Function(...)`) — see
[Workers defaults](/sql/effect-sql/mysql#workers-defaults). One
query-surface difference from Postgres: MySQL has no `RETURNING`
clause, so inserts report generated ids via `$returningId()` and
upserts use `onDuplicateKeyUpdate` — see
[Drizzle: MySQL](/sql/drizzle/mysql).
