...
for bringing the [Runtime](/infrastructure-as-effects/runtime)
...
`Platform` handles the Effect plumbing — running the user's constructor
...
After running the constructor Effect, `Platform` hands any returned `fetch`
...
The same constructor Effect runs twice — at plantime to record bindings,
...
props, a constructor Effect, a `{ fetch }` handler:
...
the constructor Effect and collects `host.run` + `fetch` into
...
- [Phases](/infrastructure-as-effects/phases) — the Construction/Runtime split
