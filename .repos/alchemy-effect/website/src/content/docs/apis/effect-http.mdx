...
nothing, which is what makes it safe at Construction/plan time
([Phases](/infrastructure-as-effects/phases)) — and each
...
Lambda Function with `functionUrl: true` serves. The `platform` Layers are
