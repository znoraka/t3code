...
2. **Bind resources and construct handlers inside the Function's
   Construction phase.** The Construction phase runs at deploy time *and* at cold
...
Now wire it up. Create `src/JobFunction.ts` with an empty Construction
...
must be bundled into the function. Provide them on the constructor Effect:
...
build inside the constructor. The handlers close over the `getItem` / `putItem`
...
The return value of the constructor is the Function's runtime surface — for an
