/**
 * True `alchemy dev` end-to-end for the React Router site: spawns the REAL
 * CLI (mirroring examples/aws-dev/test/dev.test.ts), which runs React
 * Router's own Vite dev server as the local `Website.Server` provider —
 * no Fly Machine, no Fly, no S3; the only cloud touch is the state store.
 *
 * Coverage:
 ...
 *                       (read via `process.env` in the route loader)
 *   - static assets   → `/robots.txt` from public/
 *   - HOT RELOAD      → editing app/routes/home.tsx is served by Vite's
...
const pagePath = path.join(root, "app", "routes", "home.tsx");
...
test(
  "alchemy dev serves the React Router site locally with hot reload",
  async () => {
    ...
    expect(home).toContain("Hello from React Router on Fly!");
