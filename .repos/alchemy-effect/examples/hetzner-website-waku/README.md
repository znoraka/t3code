# Hetzner Website: Waku

Deploys a [Waku](https://waku.gg) site to Hetzner with
`Hetzner.Website.Waku` — no adapter setup in the project config.
...
const site = yield* Hetzner.Website.Waku("WakuSite", {
  env: { GREETING: "Hello from Waku on Hetzner!" },
});
```
