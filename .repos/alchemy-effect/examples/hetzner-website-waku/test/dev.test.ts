/**
 * True `alchemy dev` end-to-end for the Waku site: spawns the REAL CLI
 * (mirroring examples/aws-dev/test/dev.test.ts), which runs Waku's own
 * dev server as the local `Website.Server` provider — no Hetzner Server, no
 * Hetzner, no S3; the only cloud touch is the state store.
 *
 * Coverage:
 ...
 *   - routing         → `/about` (static route) serves
 *   - static assets   → `/hello.txt` from public/
 *   - HOT RELOAD      → editing src/pages/index.tsx is served by the dev
...
test(
  "alchemy dev serves the Waku site locally with hot reload",
  async () => {
    ...
    // Dev identity: the framework dev server, not Hetzner. The port is
    ...
    expect(home).toContain("Hello from Waku on Hetzner!");
    ...
    await pollUntil(
      ...
      async () => {
        try {
          ...
          return html.includes("[dev-v2]") ? true : undefined;
        } catch {
    ...
    await pollUntil(
      ...
      async () => {
        try {
          ...
          if (!res.ok) return undefined;
          const html = await res.text();
          return html.includes("[dev-v2]") ? undefined : true;
        } catch {
          return undefined; // mid-reload
        }
      },
