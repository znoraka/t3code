...
if (!hasCreds) {
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      expect(html).toContain("Nuxt on Hetzner");
      // The `GREETING` env value from alchemy.run.ts, read via
      // `process.env` during SSR — proves the Hetzner Server rendered it at
      // request time.
      expect(html).toContain("Hello from Nuxt on Hetzner!");
    }),
    { timeout: 180_000 },
  );
