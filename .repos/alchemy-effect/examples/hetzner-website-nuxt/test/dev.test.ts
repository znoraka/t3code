/**
 * True `alchemy dev` end-to-end for the Nuxt site: spawns the REAL CLI
 * (mirroring examples/aws-dev/test/dev.test.ts), which runs Nuxt's own
...
const pagePath = path.join(root, "app", "pages", "index.vue");
const pageSource = fs.readFileSync(pagePath, "utf8");
const MARKER = "Nuxt on Hetzner";
const MARKER_V2 = "Nuxt on Hetzner [dev-v2]";
...
test(
  "alchemy dev serves the Nuxt site locally with hot reload",
  async () => {
    ...
    expect(home).toContain("Hello from Nuxt on Hetzner!");
    ...
    expect(hello).toEqual({ greeting: "Hello from Nuxt on Hetzner!" });
