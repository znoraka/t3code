# Fly Website: SvelteKit

Deploys a [SvelteKit](https://svelte.dev/docs/kit) site to Fly with
`Fly.Website.SvelteKit` — no adapter setup in the project config.
...
const site = yield* Fly.Website.SvelteKit("SvelteKitSite", {
  env: { GREETING: "Hello from SvelteKit on Fly!" },
});
```
