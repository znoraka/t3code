...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(html).toContain("SvelteKit on Fly");
    // The `GREETING` env value from alchemy.run.ts, read via `process.env`
    // in the server `load` — proves the Fly Machine rendered it at request time.
    expect(html).toContain("Hello from SvelteKit on Fly!");
  }),
  { timeout: 180_000 },
);
