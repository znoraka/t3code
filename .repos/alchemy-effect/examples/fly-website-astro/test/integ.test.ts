...
test(
  ...
  Effect.gen(function* () {
    ...
    // `process.env` in the page frontmatter — proves the Fly Machine
    // rendered it at request time.
    expect(html).toContain("Hello from Astro on Fly!");
    expect(html).toContain("server-rendered in a Fly Machine");
  }),
  { timeout: 180_000 },
);
