...
// Service (~5-10 minutes), so give the hook far more headroom than
// the default 120s.
...
test(
  ...
  Effect.gen(function* () {
    ...
    // `process.env` in the server function — proves the Railway Service
    // rendered it at request time.
    expect(html).toContain("Hello from TanStack Start on Railway!");
