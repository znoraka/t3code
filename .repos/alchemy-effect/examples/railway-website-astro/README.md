# Railway Website: Astro

Deploys an [Astro](https://astro.build) site to Railway with
`Railway.Website.Astro` — no adapter setup in the project config.
...
const site = yield* Railway.Website.Astro("Astro", {
  env: { GREETING: "Hello from Astro on Railway!" },
});
```
