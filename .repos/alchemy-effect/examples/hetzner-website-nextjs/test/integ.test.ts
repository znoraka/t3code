...
// A fresh Hetzner deployment (and the Hetzner Server behind it) can serve
...
// While the asset manifest and Hetzner edge caches are still
...
// Hetzner deployment (~5-10 minutes), so give the hook far more
...
if (!hasCreds) {
  ...
  // Deleting the Hetzner deployment takes several minutes too.
  ...
  // In-repo ONLY: bun's isolated-install store breaks Node's traced
  // node_modules when the Hetzner Server zip flattens the `next` symlink (see the
  // README) — Hetzner Server-served routes 500 with a missing @swc/helpers module.
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      expect(html).toContain("Next.js on Hetzner");
      // The `GREETING` env value from alchemy.run.ts, read via
      // `process.env` in the force-dynamic page — proves the Hetzner Server
      // rendered it at request time.
      expect(html).toContain("Hello from Next.js on Hetzner!");
    }),
    { timeout: 180_000 },
  );
