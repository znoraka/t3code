 ...
 * `[object Object]`). This suite still asserts the local URL identity;
 * body-level SSR/HMR checks require `ALCHEMY_TEST_SOLIDSTART_DEV_SSR=1`
 * (Node-hosted sidecar).
 *
 * Coverage:
 ...
 *                       Vite's HMR without a redeploy (gated)
 */
...
test(
  ...
  async () => {
    ...
    // Dev identity: the framework dev server, not Railway.
    ...
    expect(home).toContain("Hello from SolidStart on Railway!");
