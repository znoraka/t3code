...
// creates a Railway Service (~5-10 minutes), so give the hook far more
...
test(
  ...
  Effect.gen(function* () {
    ...
    // `process.env` in the index route — proves the Railway Service
    // rendered it at request time.
    expect(html).toContain("Hello from SolidStart on Railway!");
