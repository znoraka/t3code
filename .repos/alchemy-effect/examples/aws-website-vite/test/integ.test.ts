...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(html).toContain("<title>Vite on AWS</title>");
  }),
  { timeout: 180_000 },
);

test(
  "ships the React app in the client bundle",
  Effect.gen(function* () {
    const url = yield* base;
    // Client-only SPA: the greeting and Card render in the browser, so
    ...
    expect(js).toContain("Hello from Vite!");
...
test(
  ...
  Effect.gen(function* () {
    ...
    // compiled rule for a utility used in src/main.tsx only exists if the
...
test(
  "answers unmatched paths with the index page (spa default)",
  Effect.gen(function* () {
    const url = yield* base;
    // `spa` defaults on for AWS.Website.Vite: a miss serves the index
    // page with a 200 so client-side routes deep-link correctly.
