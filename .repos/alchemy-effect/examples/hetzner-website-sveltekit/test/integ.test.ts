...
if (!hasCreds) {
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      expect(html).toContain("SvelteKit on Hetzner");
      // The `GREETING` env value from alchemy.run.ts, read via `process.env`
      // in the server `load` — proves the Hetzner Server rendered it at request time.
      expect(html).toContain("Hello from SvelteKit on Hetzner!");
    }),
    { timeout: 180_000 },
  );
