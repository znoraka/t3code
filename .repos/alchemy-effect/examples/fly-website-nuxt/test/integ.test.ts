...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(html).toContain("Nuxt on Fly");
    // The `GREETING` env value from alchemy.run.ts, read via
    // `process.env` during SSR — proves the Fly Machine rendered it at
    // request time.
    expect(html).toContain("Hello from Nuxt on Fly!");
  }),
  { timeout: 180_000 },
);
