/**
 * True `alchemy dev` end-to-end for the Astro site: spawns the REAL CLI
 * (mirroring examples/aws-dev/test/dev.test.ts), which runs Astro's own
...
const MARKER = "server-rendered in a Hetzner Server";
const MARKER_V2 = "server-rendered in a Hetzner Server [dev-v2]";
...
test(
  "alchemy dev serves the Astro site locally with hot reload",
  async () => {
    ...
    expect(home).toContain("Hello from Astro on Hetzner!");
