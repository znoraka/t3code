...
if (!hasCreds) {
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      // `process.env` in the page frontmatter — proves the Hetzner Server
      // rendered it at request time.
      expect(html).toContain("Hello from Astro on Hetzner!");
      expect(html).toContain("server-rendered in a Hetzner Server");
    }),
    { timeout: 180_000 },
  );
