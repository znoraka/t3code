...
// Fly deployment (~5-10 minutes), so give the hook far more
...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(html).toContain("Next.js on Fly");
    // The `GREETING` env value from alchemy.run.ts, read via
    // `process.env` in the force-dynamic page — proves the Fly Machine
    // rendered it at request time.
    expect(html).toContain("Hello from Next.js on Fly!");
  }),
  { timeout: 180_000 },
);
