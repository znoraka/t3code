/**
 * True `alchemy dev` end-to-end for the SolidStart site: spawns the REAL
 * CLI (mirroring examples/aws-dev/test/dev.test.ts), which runs
 * SolidStart's own Vite dev server as the local `Website.Server`
 * provider — no Fly Machine, no Fly, no S3; the only cloud touch is the
 * state store.
 *
 * Coverage:
 ...
 *                       (read via `process.env` in the index route)
 *   - static assets   → `/robots.txt` from public/
 *   - HOT RELOAD      → editing src/routes/index.tsx is served by Vite's
 *                       HMR without a redeploy
 *
 * SolidStart 2's dev SSR (via `srvx/node`) misrenders under Bun. Run this
 * suite with Node (`node --experimental-strip-types` / `npx alchemy
 * dev`) if the body assertions fail under Bun.
 */
...
test(
  "alchemy dev serves the SolidStart site locally with hot reload",
  async () => {
    ...
    expect(home).toContain("Hello from SolidStart on Fly!");
