...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(html).toContain("Nuxt on Railway");
    // The `GREETING` env value from alchemy.run.ts, read via
    // `process.env` during SSR — proves the Railway Service rendered it at
    // request time.
    expect(html).toContain("Hello from Nuxt on Railway!");
  }),
  { timeout: 180_000 },
);
