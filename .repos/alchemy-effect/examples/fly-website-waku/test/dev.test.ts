 ...
 * dev server as the local `Website.Server` provider — no Fly Machine, no
 * Fly, no S3; the only cloud touch is the state store.
 *
 * Coverage:
...
test(
  ...
  async () => {
    ...
    expect(home).toContain("Hello from Waku on Fly!");
