...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(html).toContain("SvelteKit on Railway");
    // The `GREETING` env value from alchemy.run.ts, read via `process.env`
    // in the server `load` — proves the Railway Service rendered it at request time.
    expect(html).toContain("Hello from SvelteKit on Railway!");
  }),
  { timeout: 180_000 },
);
