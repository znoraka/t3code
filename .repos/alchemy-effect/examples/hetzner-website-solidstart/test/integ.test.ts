...
// creates a Hetzner Server (~5-10 minutes), so give the hook far more
...
if (!hasCreds) {
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      // `process.env` in the index route — proves the Hetzner Server
      // rendered it at request time.
      expect(html).toContain("Hello from SolidStart on Hetzner!");
