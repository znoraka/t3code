# Railway Website: React Router

Deploys a [React Router](https://reactrouter.com) v7 (framework mode)
site to Railway with `Railway.Website.ReactRouter` — no adapter setup
...
- `app/routes/home.tsx` is server-rendered on the Railway Service and reads `GREETING` from `process.env`.
...
const site = yield* Railway.Website.ReactRouter("Website", {
  env: { GREETING: "Hello from React Router on Railway!" },
});
```
