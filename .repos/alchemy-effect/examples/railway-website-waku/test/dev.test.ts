 ...
 * dev server as the local `Website.Server` provider — no Railway Service, no
 * Railway, no S3; the only cloud touch is the state store.
 *
 * Coverage:
...
test(
  ...
  async () => {
    ...
    expect(home).toContain("Hello from Waku on Railway!");
