...
// Server (~5-10 minutes), so give the hook far more headroom than the
// default 120s.
if (!hasCreds) {
  ...
  test(
    ...
    Effect.gen(function* () {
      ...
      // `process.env` in the route's loader — proves the Hetzner Server
      // rendered it at request time.
      expect(html).toContain("Hello from React Router on Hetzner!");
